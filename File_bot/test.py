...
class youhon_GlobalCollection(BASE):
    __tablename__ = "youhon_globalcollection"
    ...
    def __repr__(self):
        return "<youhon Global Collection lists '%s' for %s>" % (
            self.contents,
            self.keywoard,
        )
...
def get_collectionlist_items():
    try:
        chats = SESSION.query(
            youhon_GlobalCollection.keywoard).distinct().all()
